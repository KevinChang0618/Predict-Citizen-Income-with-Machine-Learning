...
```{r}
dat <- train[, -5]
mod <- tree(income~relationship+age+workclass+education+fnlwgt+race+capital_gain+occupation+
              capital_loss +
              marital_status+hours_per_week+sex, dat)
mod
```
...
The misclassification rate of the trainng dataset is 0.1849.


```{r}
preds = predict(mod, test, type="class")
table(preds, test$income)       
```

```{r}
mean(preds!=test$income)
```

The misclassification rate of the test dataset is 0.1779.
...
```{r}
cv=cv.tree(mod, FUN = prune.misclass)
cv
```
```{r}
plot(cv)
...
```{r}
mod.pruned = prune.misclass(mod, best=3 )
mod.pruned
```
Therefore, it can be found that 24.08% of all the people's income is greater than 50K. In the original model, capital_gain, occupation, relationship and education influence people's income to the greatest extent.By optimizing the classification of the tree, relationship and education influence people's income status to the greatest extent in the final model.
